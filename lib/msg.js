...
const downloadMediaMessage = async(m, filename) => {
	// Normalise ALL viewOnce wrappers → treat as inner media type
	if (m.type === 'viewOnceMessage' || m.type === 'viewOnceMessageV2' || m.type === 'viewOnceMessageV2Extension') {
		m.type = m.msg?.type || m.type;
	}
...
const sms = (conn, m) => {
	...
	if (m.message) {
		...
		const _isVOW = m.type === 'viewOnceMessage' || m.type === 'viewOnceMessageV2' || m.type === 'viewOnceMessageV2Extension';
		m.msg = _isVOW ? m.message[m.type].message[getContentType(m.message[m.type].message)] : m.message[m.type]
		if (m.msg) {
			if (m.type === 'viewOnceMessage' || m.type === 'viewOnceMessageV2' || m.type === 'viewOnceMessageV2Extension') {
			...
			if (m.quoted) {
				try {
					...
					const _isQVOW = m.quoted.type === 'viewOnceMessage' || m.quoted.type === 'viewOnceMessageV2' || m.quoted.type === 'viewOnceMessageV2Extension';
					m.quoted.msg = _isQVOW ? m.quoted[m.quoted.type].message[getContentType(m.quoted[m.quoted.type].message)] : m.quoted[m.quoted.type]
					if (m.quoted.type === 'viewOnceMessage' || m.quoted.type === 'viewOnceMessageV2' || m.quoted.type === 'viewOnceMessageV2Extension') {
					...
					// Never let a malformed quoted message crash the whole pipeline —
					// degrade gracefully by keeping whatever quoted info we already have.
				}
			}
		}
