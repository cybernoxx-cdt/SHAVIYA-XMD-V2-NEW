...
// ── Lazy require — avoids circular dependency at plugin load time ──
function getSetting(k)       { return require('../lib/settings').getSetting(k); }
async function setSetting(k, v)    { return require('../lib/settings').setSetting(k, v); }
function getAllSettings()     { return require('../lib/settings').getAllSettings(); }
async function resetAllSettings()  { return require('../lib/settings').resetAllSettings(); }
...
const SETTINGS_LIST = [
    ...
    { id: 10, label: 'Anti Call',           icon: '📵', key: 'antiCall',       group: 'sec'  },
    // ── UI ──
    { id: 11, label: 'Button Mode',         icon: '🔘', key: 'button',         group: 'ui'   },
    { id: 12, label: 'Movie Doc Thumb',     icon: '🎬', key: 'moviedoc',       group: 'ui'   },
    { id: 13, label: 'Always Offline',      icon: '👻', key: 'alwaysOffline',  group: 'ui'   },
];
